...
def llm_classify(
    ...
    system, user = build_classify_prompt(
        block_name=block_name, layer_name=layer_name, specification=specification, tag=tag
    )
...
def llm_classify_uncertain(
    project_id: int, eos: list = None, min_confidence: float = 0.5, limit: int = 100, object_ids: list = None
) -> dict:
        ...
        if res:
            ...
            try:
                db.update_engineering_object(
                    eo.id,
                    discipline=res["discipline"] or eo.discipline,
