...
class BindingSuggestion(BaseModel):
    ...
    selected_boq_id: str | None = Field(default=None, description="选中的 BOQ 编号（no_match=false 时必须属于候选集）")
    ...
    needs_review: bool = Field(default=False, description="模型自身不确定时置 True，候选标记「需人工复核」等待人工确认")
...
def parse_binding_suggestion(content: str, allowed_boq_ids: list = None) -> BindingSuggestion:
    ...
    if not sug.selected_or_no_match():
        raise SchemaError("输出不合规：no_match=True 时不得给出 selected/alternative；否则 selected_boq_id 必填")
    # 业务校验：选中项必须在候选集内（no_match 跳过）
    if not sug.no_match and allowed_boq_ids and sug.selected_boq_id not in allowed_boq_ids:
        raise SchemaError(f"selected_boq_id={sug.selected_boq_id} 不在候选集 {allowed_boq_ids[:5]}")
    return sug
